...
| XMSS reference-interop on keygen / sign | XMSS, XMSS-MT | 42 | **Reference-interop gap** investigated in T156 (no tests added, only documentation). `hitls_crypto::xmss::tree::compute_root` (called by `generate()` for keygen, and indirectly via `xmss_sign` for sign-side leaf walks) produces a **different `PK.root` from C / RFC 8391 reference** given the same `(SK.seed, SK.prf, PK.seed)` seeds — surfaced by trial-running GENKEY_KAT (14 rows) + SIGN_KAT success (14 rows), all 28 of which fail with byte-exact divergence. The 14 SIGN-failure rows (KEY_EXPIRED at `index == 1 << h`) work fine — they only test the state-check, not the tree contents. **Self round-trip works** (Rust `generate → sign → verify` is internally consistent, lib tests 62/0 pass; VERIFY_KAT 42/42 pass because the auth-path climb is byte-compatible with C — only the full-tree-build is broken). Same class of bug as FrodoKEM / I145. | dedicated investigation: instrument `wots_pk_gen` (leaf computation incl. L-tree compression) and `compute_root` (parent hashing + ADRS sequencing) to find where the Rust tree shape diverges from RFC 8391 §4.1.4-6 / openHiTLS C `XmssTree_ComputeNode` — likely a leaf-ordering, L-tree address or `tree_height` off-by-one issue. Once fixed, the kat-nonce hooks (`from_private_key` + `from_seeds` for both Xmss + XmssMt + `sign_at_index`) can be added and 28 SIGN-success + 14 GENKEY tests migrated |
