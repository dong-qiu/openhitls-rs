...
| SM4 HCTR / XTS / GCM-decrypt | SM4 | 11 | HCTR / XTS have no public SM4 entry point in `hitls_crypto::modes` (the existing `hctr.rs` / `xts.rs` are AES-only); GCM-decrypt needs the 16-byte tag, absent from the `.data` cipher field. | add SM4 wrappers around the existing `hctr.rs` / `xts.rs` generic paths; for GCM-decrypt the C SDV would have to publish the tag |
