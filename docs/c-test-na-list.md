...
| ~~SPAKE2+ RFC 9383 byte-exact~~ — **FULLY RESOLVED (I161/T281 → I164/T285); 5/5 vectors byte-exact** | SPAKE2+ (`auth/pake`) | **5** (`SPAKE2PLUS_TC001`); **5 migrated byte-exact** | The actual C `.data` carries **5** RFC 9383 vectors (P256-SHA256, P256-SHA512, P384-SHA256, P384-SHA512, P521-SHA512 — all HMAC; the earlier "14 across …×HMAC/CMAC" figure was an estimate that predated reading the file, which has no CMAC rows). **A-0 probe (I161) found two real RFC 9383 conformance bugs in `hitls_auth::spake2plus`** (both fixed): (1) the **key schedule** used a non-conformant `Hash(TT)[..16]` split + `HMAC(·,"ConfirmProver"/"ConfirmVerifier")` instead of RFC 9383 §3.4's `K_confirmP‖K_confirmV = HKDF(nil, K_main, "ConfirmationKeys")` + `K_shared = HKDF(nil, K_main, "SharedKey")`; (2) the transcript `TT` hardcoded **empty** `Context`/`idProver`/`idVerifier`. Both made the Rust SPAKE2+ **non-interoperable**. **Resolved (I161):** rewrote the §3.4 HKDF key schedule + added `set_identities`. **Migrated byte-exact (T281):** `kat-nonce`-gated `generate_share_with_scalar(x)` hook + P256-SHA256 vector. **A-2 increment 1 (I163/T284):** generalised off the hardcoded SHA-256 hash family (`Spake2Suite` enum + `with_suite`) → P256-SHA512 byte-exact. **A-2 increment 2 (I164/T285):** generalised off P-256 — `Spake2Suite` now carries the curve; new public `EcPoint::from_compressed` (generic SEC1 decompression for all `p ≡ 3 mod 4` curves) replaces the P-256-hardcoded decompressor; per-curve RFC 9383 §4 M/N constants (ground-truth from C `spake2plus_ecc.h`); `field_size()`-driven `w0` padding + password derivation → **P384-SHA256 / P384-SHA512 / P521-SHA512 byte-exact**. All 5 C vectors now pass byte-exact via `check_prover_byte_exact` against the **independent** C SDV vectors. The 3 T257 round-trip pins remain. | **None — byte-exact migration complete.** (If a future C `.data` adds CMAC-AES suites, add a `Spake2Suite` MAC variant + CMAC `mac()` arm; the curve/hash generalisation already in place would carry them.) |
| Privacy Pass RFC 9474 RSABSSA byte-exact — **RESOLVED (I162/T282)** | Privacy Pass (`auth/privpass_token`) | 1 (`VECTOR_TEST_TC001`); **migrated byte-exact** | The C `VECTOR_TEST` publishes RFC 9474 RSABSSA-SHA384-**PSS** (sLen=48) `token_request`/`token_response`/`token` with a stubbed RNG injecting `nonce` (32) → `salt` (48) → `blind` r (256). The Rust `hitls_auth::privpass` was a **simplified blind-RSA** that blinded `SHA-256(token_input)` directly (no EMSA-PSS) → non-interoperable. **Resolved (I162):** rewrote the blind to RFC 9474 — `m = OS2IP(EMSA-PSS-ENCODE(SHA384(token_input), modBits-1, salt))` with SHA-384/MGF1-SHA-384/sLen=48; `verify_token` now does `RSASSA-PSS-VERIFY` (SHA-384, sLen=48); exposed `hitls_crypto::rsa::emsa_pss_encode` (un-gated `pss_sign_pad_with_salt_bytes_alg`); added `Client::with_token_key_id` (RFC 9578 §6.5 `token_key_id = SHA256(DER SPKI)`) + `verify_token_with_key_id`. **Migrated byte-exact (T282):** `kat-nonce`-gated `create_token_request_with_randomness(challenge, nonce, salt, blind)` + `tc_privpass_rfc9474_vector_byte_exact` asserts `blinded_msg` / `blind_sig` / `authenticator` byte-exact + the token verifies — all against the **independent** C vector (true RFC 9474/9578 ground-truth; the variant — PSS sLen=48 + emBits=modBits-1 — was reverse-engineered from the RNG-stub order via an independent Python reference). The 5 T256 round-trip pins remain. | **Remaining:** ~~RFC 9577 TokenChallenge `serialize`/`deserialize`~~ **RESOLVED (I166/T288)** — new `hitls_auth::privpass::wire` module with byte-exact serialize/deserialize for all 5 RFC 9577/9578 wire structures (TokenChallengeRequest / TokenChallenge / TokenRequest / TokenResponse / Token); `migrated_privpass_serialization.rs` migrates `SERIALIZATION_TC001` (19 byte-exact round-trips) + `SERIALIZATION_INVALID_TC002` (33 malformed-reject) byte-exact. Still open (minor): production `token_key_id = SHA256(SPKI)` SPKI-DER builder (the Client takes a precomputed key id today) + `SERIALIZATION_INVALID_TC003` (C type-aware serialize-validation, out of scope for the type-agnostic wire serializer). |
