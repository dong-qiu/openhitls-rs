...
| Privacy Pass RFC 9474 RSABSSA byte-exact | Privacy Pass (`auth/privpass_token`) | 1 (`VECTOR_TEST_TC001`) | The C `VECTOR_TEST` publishes RFC 9474 RSABSSA `token_request` / `token_response` / `token` produced with **EMSA-PSS** encoding + a stubbed RNG injecting fixed `nonce` / `salt` / `blind`. The Rust `hitls_auth::privpass` is a **simplified blind-RSA** that blinds `SHA-256(token_input)` directly — no EMSA-PSS, no salt, no RFC 9577 TokenChallenge wire codec, and draws `nonce` / `blind` internally via `getrandom`. So the C `request` / `response` / `token` bytes are not reproducible. **What IS migrated (T256, round-trip):** the C vector's actual RSA-2048 key + challenge drive the Rust Issuer/Client/verify pipeline end-to-end (`migrated_privpass.rs`, 5 round-trip + negative pins; SM9/T158 methodology — proves the vector's key material is usable, not byte-exact KAT). | **Unblock (future Implementation phase):** add RFC 9474 EMSA-PSS RSABSSA to `hitls_auth::privpass` (blind the PSS-encoded message, not the raw hash) + `kat-nonce`-gated deterministic `nonce` / `salt` / `blind` hooks + RFC 9577 TokenChallenge `serialize`/`deserialize`; then the VECTOR `request`/`response`/`token` migrate byte-exact. |
