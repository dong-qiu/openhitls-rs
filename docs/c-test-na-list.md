...
| FrodoKEM / Classic McEliece decaps KAT | FrodoKEM, McEliece | 28 (frodokem 26 + mceliece 2) | **Confirmed reference-interoperability bug (T146 + follow-up diagnosis).** The decaps KATs (`ENCAPS_DECAPS_FUNC_TC001`: load `testDk`, check `decapsulate(testCt) == testSs`) cannot be migrated because **FrodoKEM/McEliece are not reference-interoperable**: decapsulating a *reference* `(sk, ct)` pair does not recover the reference shared secret. Diagnosed precisely for **FrodoKEM-640-SHAKE**: (1) the secret-key byte layout's `s`/`pk`/`pkh` portions are **reference-correct** — byte-verified that `testEk == testDk[16..16+9616]` (pk at offset 16) and `testDk[-16:] == SHAKE128(testEk)[:16]` (pkh); (2) Rust decaps falls into the **implicit-rejection branch** — its output equals `SHAKE(ct ‖ s)` (the FO reject value), not `testSs` — meaning the PKE-decrypt + re-encrypt step fails to reconstruct the reference ciphertext. So the divergence is in the **S^T secret-matrix interpretation** (Rust reads `testDk[9632..19872]` as row-major little-endian `u16`; the reference packing/transpose convention likely differs) **or the PKE-decrypt matrix math** — *not* the gross sk layout or sizes (FrodoKEM-640 dk = 19888, McEliece-6688128 dk = 13932 both match). McEliece errors inside `decode` (same class of issue). **Why it hid:** both impls are validated only by self round-trip (`generate → encaps → decaps`, asserting `ss1 == ss2`), so they are internally consistent but were never proven reference-compatible. **A follow-up empirically RULED OUT the simple-transpose hypothesis:** re-building `testDk` with the S^T region transposed (`rust_st[j*n+i] = ref[i*n_bar+j]`, matching Rust's keygen transpose) still decaps to the reject value, not `testSs`. So the divergence is *deeper* than the S^T flat layout — it is in the PKE pipeline itself: the matrix-multiply orientation (`mul_bs` / `mul_add_as_plus_e`), the ciphertext `c1`/`c2` pack/unpack, the `A`-matrix expansion order from `seedA`, and/or `encode`/`decode`. (C samples S^T directly via `FrodoCommonSampleNFromR`, whereas Rust samples `S` then transposes — so the two keygens already produce different B for the same seed; the divergence is broader than a single field.) | dedicated deep-dive: step-by-step compare each FrodoKEM PKE stage (A-expansion, pack/unpack, the three matrix-mul orientations, noise sampling order, encode/decode) Rust↔C with intermediate-value checks; realign Rust to the C/reference convention, then re-verify all 6 FrodoKEM param sets + self round-trip + the C KAT vectors (and the McEliece equivalent). Substantial, multi-step production PQC change — simple S^T transpose already ruled out |
