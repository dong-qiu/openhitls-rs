...
| SM4 HCTR / XTS-stealing reference-interop + GCM-decrypt data | SM4 | 9 | **Partially resolved in I156.** Added `sm4_xts_encrypt` / `sm4_xts_decrypt` (`modes::xts`) and `sm4_hctr_encrypt` / `sm4_hctr_decrypt` (`modes::hctr`) by extracting generic `*_inner<C: BlockCipher>` paths shared with the AES wrappers — 2 block-aligned (`input.len() == 16`) SM4-XTS rows now match the C SDV byte-exact. **Remaining 9 unsupported rows** split across two categories: (i) **reference-interop divergence** — the longer SM4-XTS rows (those that trigger the IEEE 1619 ciphertext-stealing path; 4 rows) and **all** SM4-HCTR rows (2) fail byte-exact against the C reference. The Rust XTS-stealing path and the Rust HCTR `uhash` are internally self-consistent (encrypt/decrypt round-trip cleanly under proptest + IEEE P1619 vector 1 passes for AES-XTS aligned) but were never proven byte-compatible with the openHiTLS C reference. Same diagnostic flavor as the pre-I145 FrodoKEM gap (Rust `pack` / `unpack` bit-endianness diverged from C). (ii) **C SDV data limitation** — the SM4-GCM-decrypt rows lack the 16-byte auth tag in the `.data` cipher field (only ciphertext is published), so byte-exact decrypt cannot authenticate. | (i) instrument `xts::xts_*_inner` ciphertext-stealing path + `hctr::uhash` side-by-side with C `EAL_SM4` to pin the divergence (same playbook as the I145 FrodoKEM `pack`/`unpack` debug); (ii) the C SDV would need to publish tags for the decrypt rows. |
