...
| SPAKE2+ RFC 9383 byte-exact — **P256-SHA256 RESOLVED (I161/T281); P256-SHA512 RESOLVED (I163/T284); P-384/P-521 + CMAC suites pending** | SPAKE2+ (`auth/pake`) | 14 (`SPAKE2PLUS_TC001`); **2 migrated byte-exact** | The C `.data` carries 14 RFC 9383 vectors across suites P-256/P-384/P-521 × SHA-256/512 × HMAC/CMAC-AES. **A-0 probe (I161) found two real RFC 9383 conformance bugs in `hitls_auth::spake2plus`** (both fixed): (1) the **key schedule** used a non-conformant `Hash(TT)[..16]` split + `HMAC(·,"ConfirmProver"/"ConfirmVerifier")` instead of RFC 9383 §3.4's `K_confirmP‖K_confirmV = HKDF(nil, K_main, "ConfirmationKeys")` + `K_shared = HKDF(nil, K_main, "SharedKey")`; (2) the transcript `TT` hardcoded **empty** `Context`/`idProver`/`idVerifier` with no way to set them. Both made the Rust SPAKE2+ **non-interoperable** with conformant peers. **Resolved (I161):** rewrote the §3.4 HKDF key schedule + added `set_identities(context, idProver, idVerifier)`; the share computation (`shareP = x·G + w0·M`, RFC M/N points) was already conformant. **Migrated byte-exact (T281):** a new `kat-nonce`-gated `generate_share_with_scalar(x)` hook injects the vector's scalar; `tc_spake2plus_rfc9383_p256_vector_byte_exact` asserts `shareP` / `K_shared` / `confirmP` byte-exact + `confirmV` verifies — all against the **independent** C vector (true RFC 9383 ground-truth). **A-2 (I163/T284):** generalised off the hardcoded SHA-256 hash family — new `Spake2Suite` enum (`P256Sha256` / `P256Sha512`) + `Spake2Plus::with_suite(role, suite)`; the §3.4 key schedule + confirmation MACs are now parameterised over the suite's `hlen`/hash/HMAC. `tc_spake2plus_rfc9383_p256_sha512_vector_byte_exact` migrates the `P256-SHA512-HKDF-SHA512-HMAC-SHA512` vector byte-exact (`shareP` / 64-byte `K_shared` / `confirmP` + `confirmV` verifies). The 3 T257 round-trip pins remain. | **Remaining (future Implementation phase):** the curve is still P-256 only — generalise the M/N points + `decompress_point` for P-384/P-521 (the 6 P-384/P-521 vectors) and add a CMAC-AES MAC suite (the 2 CMAC vectors); then they migrate byte-exact through the same `with_suite` + `generate_share_with_scalar` hooks. |
| Privacy Pass RFC 9474 RSABSSA byte-exact — **RESOLVED (I162/T282)** | Privacy Pass (`auth/privpass_token`) | 1 (`VECTOR_TEST_TC001`); **migrated byte-exact** | The C `VECTOR_TEST` publishes RFC 9474 RSABSSA-SHA384-**PSS** (sLen=48) `token_request`/`token_response`/`token` with a stubbed RNG injecting `nonce` (32) → `salt` (48) → `blind` r (256). The Rust `hitls_auth::privpass` was a **simplified blind-RSA** that blinded `SHA-256(token_input)` directly (no EMSA-PSS) → non-interoperable. **Resolved (I162):** rewrote the blind to RFC 9474 — `m = OS2IP(EMSA-PSS-ENCODE(SHA384(token_input), modBits-1, salt))` with SHA-384/MGF1-SHA-384/sLen=48; `verify_token` now does `RSASSA-PSS-VERIFY` (SHA-384, sLen=48); exposed `hitls_crypto::rsa::emsa_pss_encode` (un-gated `pss_sign_pad_with_salt_bytes_alg`); added `Client::with_token_key_id` (RFC 9578 §6.5 `token_key_id = SHA256(DER SPKI)`) + `verify_token_with_key_id`. **Migrated byte-exact (T282):** `kat-nonce`-gated `create_token_request_with_randomness(challenge, nonce, salt, blind)` + `tc_privpass_rfc9474_vector_byte_exact` asserts `blinded_msg` / `blind_sig` / `authenticator` byte-exact + the token verifies — all against the **independent** C vector (true RFC 9474/9578 ground-truth; the variant — PSS sLen=48 + emBits=modBits-1 — was reverse-engineered from the RNG-stub order via an independent Python reference). The 5 T256 round-trip pins remain. | **Remaining:** RFC 9577 TokenChallenge `serialize`/`deserialize` + production `token_key_id = SHA256(SPKI)` (the Client takes a precomputed key id today; building the `id-RSASSA-PSS` SPKI DER in `hitls-auth` is a follow-up). |
