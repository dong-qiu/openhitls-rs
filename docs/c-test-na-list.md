...
| FrodoKEM / Classic McEliece decaps KAT | FrodoKEM, McEliece | 28 (frodokem 26 + mceliece 2) | **Reference-KAT validation gap (T146).** The decaps KATs (`ENCAPS_DECAPS_FUNC_TC001`: load `testDk`, check `decapsulate(testCt) == testSs`) cannot be migrated: loading a C-vector secret key and decapsulating produces the **wrong** shared secret (FrodoKEM returns a different ss; McEliece errors inside decode). Field lengths match the spec (e.g. FrodoKEM-640 dk = 19888, McEliece-6688128 dk = 13932), so the divergence is in the **secret-key byte layout** (and possibly the algorithm), not the sizes. Root cause: the Rust FrodoKEM/McEliece impls are validated **only by self round-trip** (`generate → encaps → decaps`), never against reference/NIST KAT vectors — so their on-the-wire sk/ct/ss encoding was never proven reference-compatible. A `from_decapsulation_key` constructor was prototyped (mirroring ML-KEM) and works for Rust-generated keys, but not for the C vectors. | dedicated investigation: byte-align the FrodoKEM/McEliece secret-key (and ciphertext) serialization with the reference, or confirm/fix an algorithmic divergence — then the decaps KATs (and a `from_decapsulation_key` constructor) can land. Larger than a localized fix; touches production PQC code |
