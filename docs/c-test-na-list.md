...
| SPAKE2+ RFC 9383 byte-exact | SPAKE2+ (`auth/pake`) | 14 (`SPAKE2PLUS_TC001`) | The C `.data` carries 14 RFC 9383 vectors across suites P-256/P-384/P-521 × SHA-256/512 × HMAC/CMAC-AES, each pinning the ephemeral scalars `x` / `y` (via a stubbed RNG) so `shareP` / `shareV` / `kShared` / `confirmP` / `confirmV` are reproducible. The Rust `hitls_auth::spake2plus` supports **only** the P-256 + SHA-256 + HMAC-SHA-256 suite (group / hash / MAC hardcoded) and `generate_share` draws the ephemeral scalar internally via the DRBG with no injection hook. So 13/14 suites are unsupported and even the P-256 suite's share/confirm bytes are not reproducible. **What IS migrated (T257, round-trip):** the P-256 vector's registration triple `(w0, w1, L)` drives a full Rust prover↔verifier exchange (`migrated_spake2plus.rs`, 3 round-trip + negative pins; SM9/T158 methodology) — the round-trip succeeding proves the RFC 9383 `(w0, w1, L)` triple is self-consistent (`L = w1·G`) under the Rust math. | **Unblock (future Implementation phase):** generalise `hitls_auth::spake2plus` to multi-suite (P-384/P-521 + SHA-512 + CMAC-AES) + add a `kat-nonce`-gated `generate_share_with_scalar(x)` hook + verify the RFC 9383 KDF / confirmation transcript construction matches byte-exact; then the 14 `shareP`/`shareV`/`kShared`/`confirmP`/`confirmV` vectors migrate. |
