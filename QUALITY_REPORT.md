...
└── Coverage         cargo-tarpaulin → Cobertura XML (added Phase T117)
```
...
|-----------|---------------|:-------------:|
| Zeroize on drop | All secret types (keys, intermediate states) | Compile-time (derive) + 4 runtime drop verification tests (Phase T164) |
| Constant-time comparison | `subtle::ConstantTimeEq` in all crypto comparisons | 6 statistical timing tests (Welch's t-test, Phase T161) |
| Unsafe code confinement | ~44 blocks: AES-NI (8), AES-NEON (6), SHA-2 HW (8), GHASH HW (22), ChaCha20 SIMD (15), McEliece (2) | NIST/RFC vectors + 8 HW↔SW cross-validation tests (Phase T159) |
...
### 1.7 High-Risk Zero Direct Unit Test Files — **SIGNIFICANTLY REDUCED** (Phase T157–T158)

Phase T157 added 15 state guard + I/O edge case tests and Phase T158 added 15 TLS 1.2 handshake + post-HS auth tests, directly covering the previously untested connection code:

| File | Lines | Risk | Coverage Mechanism |
|------|------:|:----:|-------------------|
| `hitls-tls/src/macros.rs` | 1,417 | **Low** | Phase T157: 15 state guard tests (write/read before handshake, key_update limits, shutdown states) |
| `hitls-tls/src/connection12/client.rs` | 1,025 | **Low** | Phase T158: 15 handshake edge case tests (EKM, session resumption, verify_data, MFL) |
| `hitls-tls/src/connection12/server.rs` | 927 | **Low** | Phase T158: server handshake tests (no shared cipher, cert request optional) |
| `hitls-tls/src/connection/server.rs` | 369 | **Low** | Phase T157: state guard tests + Phase T158: post-HS cert auth tests |
| `hitls-tls/src/connection/client.rs` | 197 | **Low** | Phase T157: connection_info, peer_certificates, negotiated_alpn tests |
| `hitls-crypto/src/provider.rs` | 144 | **Low** | Trait definitions; compile-time coverage |

These files now have **30 dedicated unit tests** via Phase T157–T158, covering state transitions, error paths, and edge cases. Risk reduced from Critical → Low.
...
────────────     ──   ──────────────────────────────────────────   ──────────────────────────
CLOSED           D1   0-RTT replay protection: +8 tests             Resolved (Phase T98)
CLOSED           D2   Async TLCP/DTLCP: zero tests                   Resolved (Phase T110: +15)
CLOSED           D3   Extension negotiation: +14 e2e tests           Resolved (Phase T111: +14)
MOSTLY CLOSED    D4   DTLS loss/resilience: +30 tests                Resolved (Phase T112/123/124)
MOSTLY CLOSED    D5   TLCP double certificate: +25 tests             Resolved (Phase T113/119)
CLOSED           D6   Property-based testing: +20 proptest           Resolved (Phase T117)
CLOSED           D7   Code coverage metrics in CI                    Resolved (Phase T117: tarpaulin)
PARTIALLY CLOSED D8   Cross-implementation interop (Phase T165)      OpenSSL s_client/s_server tests; TLS 1.2 verify_data gap found
Low-Med          D9   Fuzz targets: parse-only                      Deep bugs missed
MOSTLY CLOSED    D10  Crypto files without unit tests                Resolved (Phase T114–T116/125)
──────── Phase T149 深度分析 → Phase T157–T165 Quality Improvement ────────
MOSTLY CLOSED    D11  Semantic/state-machine fuzz (Phase T150/T165)  4 semantic targets; DTLS state machine fuzz added
PARTIALLY CLOSED D12  Side-channel timing tests (Phase T161)         6 timing tests added (Welch's t-test); more algorithms needed
CLOSED           D13  TLS connection code unit tests (Phase T157/153) +30 state guard + handshake edge case tests
MOSTLY CLOSED    D14  Proptest scope expanded (Phase T160)           5/9 crates covered (was 2/9)
PARTIALLY CLOSED D15  Concurrency stress tests (Phase T162)          +10 multi-threaded stress tests (48 total)
CLOSED           D16  HW↔SW cross-validation (Phase T159)           +8 differential tests across all HW-accel modules
CLOSED           D17  Zeroize runtime verification (Phase T164)      +4 drop-based memory zeroing tests
CLOSED           D18  Feature flag smoke tests (Phase T163)          +4 smoke tests for feature subsets
```

### 2.2 D1 — 0-RTT Replay Protection ~~(Critical)~~ — **CLOSED** (Phase T98)

**Resolved**: Phase T98 added 8 tests covering:
...
### 2.3 D2 — Async/Sync Test Coverage Asymmetry ~~(Critical)~~ — **CLOSED** (Phase T110)

**Resolved**: Phase T109 added 10 async TLS 1.2 tests. Phase T110 created async connection types for TLCP and DTLCP and added 15 tests (8 TLCP + 7 DTLCP). All 5 protocol variants now have async tests.
...
### 2.4 D3 — Extension Negotiation ~~(High)~~ — **CLOSED** (Phase T111)

Phase T111 added 14 E2E tests covering all identified gaps:
...
### 2.5 D4 — DTLS Loss/Retransmission ~~(High)~~ — **MOSTLY CLOSED** (Phase T112/123/124)

**Resolved across 3 phases** with 30 total DTLS-specific tests:

- **Phase T112** (+10): Post-handshake adverse delivery — out-of-order, selective loss, stale records, corrupted ciphertext, truncated/empty datagrams, wrong epoch, interleaved bidirectional
- **Phase T122** (+10): Fragmentation reassembly (multi-message, old message, out-of-order, overlapping fragments) + retransmission timer (backoff, reset cycles, timeout cap, Flight clone)
- **Phase T123** (+10): Codec edge cases (all handshake types, fragment offset, TLS↔DTLS roundtrip, HVR empty/max cookie) + anti-replay window boundaries (uninitialized, near u64::MAX, shift by WINDOW_SIZE, reset/reuse)
...
### 2.6 D5 — TLCP Double Certificate ~~(High)~~ — **MOSTLY CLOSED** (Phase T113/119)

**Resolved across 2 phases** with 25 total TLCP-specific tests:

- **Phase T113** (+10): Missing encryption certificate, missing signing key, wrong key type — unit + integration for both TLCP and DTLCP
- **Phase T118** (+15): SM3 transcript hash correctness (empty hash, incremental, hash_len), SM3-PRF determinism and cross-validation, TLCP key schedule (master secret, CBC/GCM key blocks), verify_data SM3 client/server, EMS → key block pipeline, seed order sensitivity, full SM3 verify pipeline
...
### 2.7 D6 — ~~No Property-Based Testing~~ (Medium) — **CLOSED** (Phase T117)

**Resolved**: Phase T117 added 20 proptest property-based tests:
...
### 2.8 D7 — ~~No Code Coverage Metrics~~ (Medium) — **CLOSED** (Phase T117)

**Resolved**: Phase T117 added a `cargo-tarpaulin` coverage CI job:
...
### 2.9 D8 — Cross-Implementation Interop ~~(Medium)~~ — **PARTIALLY CLOSED** (Phase T165)

**Phase T165** added OpenSSL CLI interop tests (`tests/interop/tests/openssl_interop.rs`):
...
### 2.11 D10 — ~~30~~ ~14 Crypto Files Without Direct Unit Tests ~~(Low)~~ — **MOSTLY CLOSED** (Phase T114–T116/125)

**Resolved across 4 phases** with 50 total internal module tests:

- **Phase T114** (+15): SM9 tower fields — Fp2 (add/sub/mul/frobenius/serialization), Fp4 (add/mul_by_v/frobenius/serialization), Fp12 (mul/frobenius/serialization/inverse)
- **Phase T115** (+15): SLH-DSA internals — address fields, WOTS+ chain/checksum, FORS tree/sign, hypertree sign/verify, hash PRF/H_msg
- **Phase T116** (+15): McEliece poly/matrix/syndrome, FrodoKEM matrix/pack/PKE, XMSS tree/WOTS/address
- **Phase T124** (+5): SLH-DSA WOTS+ base_b (2-bit/1-bit extraction, empty, all-zeros/all-FF checksum)
...
### 2.12 D11 — Semantic/State-Machine Fuzz — **MOSTLY CLOSED** (Phase T150 + T165)

**Phase T150** added 3 semantic fuzz targets, and **Phase T165** added 1 DTLS state machine fuzz target:
...
### 2.13 D12 — Side-Channel/Timing Test Infrastructure ~~(Critical)~~ — **PARTIALLY CLOSED** (Phase T161)

**Phase T161** added 6 statistical timing tests using Welch's t-test analysis (`crates/hitls-crypto/tests/timing.rs`):
...
### 2.14 D13 — TLS Connection Code Unit Tests ~~(Critical)~~ — **CLOSED** (Phase T157 + T158)

**Phase T157** added 15 state guard + I/O edge case tests and **Phase T158** added 15 TLS 1.2 handshake + post-HS auth edge case tests:

**Phase T157 tests** (state guards, appended to `connection/tests.rs`):
...
**Phase T158 tests** (TLS 1.2 + post-HS auth):
...
### 2.15 D14 — Proptest Scope ~~Too Narrow~~ Expanded ~~(High)~~ — **MOSTLY CLOSED** (Phase T160)

**Phase T160** expanded proptest from 2/9 to 5/9 crates:
...
### 2.16 D15 — Concurrency Testing ~~Minimal~~ Expanded ~~(High)~~ — **PARTIALLY CLOSED** (Phase T162)

**Phase T162** added 10 multi-threaded stress tests (`tests/interop/tests/concurrency.rs`):
...
### 2.17 D16 — Hardware↔Software Cross-Validation ~~(High)~~ — **CLOSED** (Phase T159)

**Phase T159** added 8 differential tests comparing HW-accelerated and SW fallback paths:
...
### 2.18 D17 — Zeroize Runtime Verification ~~(Medium)~~ — **CLOSED** (Phase T164)

**Phase T164** added 4 drop-based memory zeroing verification tests (`crates/hitls-crypto/tests/zeroize_verify.rs`):
...
### 2.19 D18 — Feature Flag Combinations ~~Untested~~ ~~(Medium)~~ — **CLOSED** (Phase T163)

**Phase T163** added 4 feature flag smoke tests (`crates/hitls-crypto/tests/feature_smoke.rs`):
...
Phase T109         +10      D2           Async TLS 1.2 deep coverage                 ✅
Phase T110         +15      D2           Async TLCP + DTLCP connection tests         ✅
Phase T111         +14      D3           Extension negotiation e2e tests             ✅
...
Phase T117         +20      D6/D7        Infra: proptest + coverage CI               ✅
Phase T118         +15      D5           TLCP SM3 cryptographic path coverage        ✅
Phase T119         +15      —            TLS 1.3 key schedule & HKDF robustness      ✅
Phase T120         +15      —            Record layer encryption & AEAD failures     ✅
Phase T121         +15      D4           TLS 1.2 CBC padding + DTLS parsing          ✅
Phase T122         +15      D4           DTLS fragmentation + retransmission         ✅
Phase T123         +15      D4           DTLS codec + anti-replay boundaries         ✅
Phase T124         +15      D10          X.509 extensions + WOTS+ + ASN.1 tags       ✅
Phase T125         +15      —            PKI encoding + signing dispatch + builder   ✅
Phase T126         +15      —            X.509 cert parsing + SM9 G2 + pairing       ✅
Phase T127         +13      —            SM9 hash + algorithm helpers + curve params ✅
Phase T128         +15      —            McEliece keygen + encoding + decoding       ✅
Phase T129         +10      —            XMSS tree + WOTS+ deepening + FORS          ✅
Phase T130         +15      —            McEliece GF + Benes + matrix deepening      ✅
Phase T131         +12      —            FrodoKEM matrix + SLH-DSA hypertree + poly  ✅
Phase T132         +15      —            McEliece + FrodoKEM + XMSS params deepening ✅
Phase T133         +15      —            XMSS hash + address + ML-KEM NTT deepening  ✅
Phase T134         +15      —            BigNum CT + primality + core type deepening  ✅
Phase T140         +15      —            SLH-DSA params + hash abstraction + address  ✅
Phase T142         +15      —            FrodoKEM PKE + SM9 G1 + SM9 Fp deepening     ✅
Phase T143         +15      —            ML-DSA NTT + SM4-CTR-DRBG + BigNum random    ✅
Phase T144         +15      —            DH group params + entropy pool + SHA-1        ✅
Phase T146         +15      —            ML-KEM poly + SM9 Fp12 + encrypted PKCS#8     ✅
Phase T147         +15      —            ML-DSA poly + X.509 extensions + PKI text     ✅
Phase T148         +15      —            XTS mode + Edwards curve + GMAC deepening     ✅
Phase T149         +15      —            scrypt + CFB mode + X448 deepening            ✅
Phase T150         +3 fuzz  D11          Semantic fuzz: AEAD + X.509 + handshake deep  ✅
──────── Quality Improvement Roadmap (Phase T157–T165) ────────
Phase T157         +15      D13          TLS connection state guards + I/O edge cases  ✅
Phase T158         +15      D13          TLS 1.2 handshake + post-HS auth edge cases   ✅
Phase T159         +8       D16          HW↔SW cross-validation differential tests     ✅
Phase T160         +15      D14          Proptest expansion (tls + pki + bignum)        ✅
Phase T161         +6       D12          Side-channel timing test infrastructure        ✅
Phase T162         +10      D15          Concurrency stress tests                       ✅
Phase T163         +4       D18          Feature flag combination smoke tests           ✅
Phase T164         +4       D17          Zeroize runtime verification                   ✅
Phase T165         +2+1fuzz D8/D11       OpenSSL interop + DTLS state machine fuzz     ✅
```

**Result**: 2,585 → 3,264 tests (+679), 14 fuzz targets, all planned deficiencies addressed or significantly reduced.

### 3.2 Phase T98 — 0-RTT Early Data + Replay Protection (~8 tests) ✅
...
### 3.3 Phase T109 — Async TLS 1.2 Deep Coverage (+10 tests) ✅

**Deficiency**: D2 (async TLS 1.2 now has 28 tests; D2 fully closed in Phase T110)
...
### 3.4 Phase T110 — Async TLCP + DTLCP Connection Tests (+15 tests) ✅
...
### 3.5 Phase T111 — Extension Negotiation E2E Tests (+14 tests) ✅
...
### 3.6 Phase T112 — DTLS Loss Simulation + Retransmission (+10 tests) ✅

**Deficiency**: D4 (High) — partially addressed (further addressed by Phase T122/124)
...
### 3.7 Phase T113 — TLCP Double Certificate Validation (+10 tests) ✅

**Deficiency**: D5 (High) — partially closed (further addressed by T111)
...
### 3.8 Phase T114 — SM9 Tower Fields (+15 tests) ✅
...
### 3.9 Phase T115 — SLH-DSA Internal Modules (+15 tests) ✅
...
### 3.10 Phase T116 — McEliece + FrodoKEM + XMSS Internals (+15 tests) ✅
...
### 3.11 Phase T117 — Infrastructure: proptest + Coverage CI (+20 tests) ✅
...
### 3.12 Phase T118–T134, T140–T149 — Deep Edge-Case Coverage (+495 tests) ✅

Continued hardening beyond the original roadmap:

| Phase | Tests | Focus |
|-------|:-----:|-------|
| T118 | +15 | TLCP SM3 transcript hash, PRF, key schedule, verify_data (closes D5 SM3 gap) |
| T119 | +15 | TLS 1.3 key schedule SHA-384 pipeline, stage enforcement, SM4-GCM-SM3 |
| T120 | +15 | Record layer AEAD encryption edge cases, failure modes, epoch transitions |
| T121 | +15 | TLS 1.2 CBC padding oracle, DTLS record parsing, TLS 1.3 inner plaintext |
| T122 | +15 | DTLS fragmentation/reassembly, retransmission timer, CertificateVerify (extends D4) |
| T123 | +15 | DTLS codec all types, anti-replay window boundaries, entropy conditioning (extends D4) |
| T124 | +15 | X.509 extension parsing, SLH-DSA WOTS+ base conversion, ASN.1 tag long-form (extends D10) |
| T125 | +15 | PKI shared encoding helpers, X.509 signing hash dispatch, certificate builder DER encoding |
| T126 | +15 | X.509 certificate parsing, SM9 G2 point arithmetic, SM9 pairing helpers |
| T127 | +13 | SM9 hash functions H1/H2/KDF, SM9 algorithm sign/verify/encrypt/decrypt, BN256 curve parameters |
| T128 | +15 | McEliece keygen helpers (bitrev/SHAKE256/PRG), encoding (error vector), decoding (Berlekamp-Massey) |
| T129 | +10 | XMSS tree operations (compute_root/sign/verify), WOTS+ deepening, SLH-DSA FORS deepening |
| T130 | +15 | McEliece GF(2^13) field algebra, Benes network permutation/sort, binary matrix Gaussian elimination |
| T131 | +12 | FrodoKEM matrix ops, SLH-DSA hypertree, McEliece polynomial deepening |
| T132 | +15 | McEliece + FrodoKEM + XMSS parameter set validation deepening |
| T133 | +15 | XMSS hash abstraction + XMSS address scheme + ML-KEM NTT deepening |
| T134 | +15 | BigNum constant-time + primality testing + core type deepening |
| T140 | +15 | SLH-DSA params + hash abstraction + address scheme deepening |
| T142 | +15 | FrodoKEM PKE + SM9 G1 point + SM9 Fp field deepening |
| T143 | +15 | ML-DSA NTT + SM4-CTR-DRBG + BigNum random deepening |
| T144 | +15 | DH group params + entropy pool + SHA-1 deepening |
| T146 | +15 | ML-KEM poly + SM9 Fp12 + encrypted PKCS#8 deepening |
| T147 | +15 | ML-DSA poly + X.509 extensions + X.509 text deepening |
| T148 | +15 | XTS mode + Edwards curve + GMAC deepening |
| T149 | +15 | scrypt + CFB mode + X448 deepening |

---

## 4. Coverage Targets — Final Status

| Metric | Original (T97) | Target (T117) | Actual (T149) | **Actual (T165)** |
|--------|:---------------:|:-------------:|:-------------:|:-----------------:|
...
All original targets met or exceeded. Quality Improvement Roadmap (T157–T165) closed or partially closed all remaining deficiencies.

---

## 5. Priority Improvement Roadmap

### 5.1 Overview — Post-T165 Status

```
Priority   Deficiency   Status            Phase     Result
────────   ──────────   ────────────────  ────────  ──────────────────────────────
P0         D13          CLOSED            T157/153  +30 connection unit tests
P0         D11          MOSTLY CLOSED     T150/160  4 semantic + DTLS fuzz targets
P1         D12          PARTIALLY CLOSED  T161      6 timing tests (Welch's t-test)
P1         D16          CLOSED            T159      8 HW↔SW differential tests
P1         D14          MOSTLY CLOSED     T160      Proptest in 5/9 crates (was 2/9)
P2         D15          PARTIALLY CLOSED  T162      +10 stress tests (48 total)
P2         D8           PARTIALLY CLOSED  T165      OpenSSL interop (TLS 1.3 ✅, TLS 1.2 known gap)
P2         D18          CLOSED            T163      4 feature smoke tests
P3         D17          CLOSED            T164      4 zeroize drop verification tests
P3         D4r          OPEN              —         Requires handshake driver refactoring
```

### 5.2 Completed Actions (Phase T157–T165)

All P0, P1, P2, and P3 actions have been addressed:

| Priority | Deficiency | Action | Phase | Result |
|----------|-----------|--------|-------|--------|
| P0 | D13 | TLS connection unit tests | T157/T158 | +30 tests (state guards + handshake edge cases) |
| P0 | D11 | Semantic + DTLS fuzz | T150/T165 | 4 semantic targets + DTLS codec fuzz |
| P1 | D12 | Timing test infrastructure | T161 | 6 Welch's t-test timing tests |
| P1 | D16 | HW↔SW cross-validation | T159 | 8 differential tests across all HW modules |
| P1 | D14 | Proptest expansion | T160 | 5/9 crates now have proptest coverage |
| P2 | D15 | Concurrency stress | T162 | +10 multi-threaded stress tests (48 total) |
| P2 | D8 | OpenSSL interop | T165 | TLS 1.3 pass; TLS 1.2 verify_data gap found |
| P2 | D18 | Feature flag testing | T163 | 4 feature subset smoke tests |
| P3 | D17 | Zeroize verification | T164 | 4 drop-based zeroing tests |
...
### 5.4 Quantified Gap Summary — Post-T165

| Metric | Before (T149) | After (T165) | Change |
|--------|:-------------:|:------------:|:------:|
