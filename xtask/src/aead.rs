...
// ---------------------------------------------------------------------------
// SipHash — (algId, key, data, mac). Both SIPHASH64 (`SipHash::hash`) and
// SIPHASH128 (`SipHash128::hash`) variants are migrated.
...
pub fn emit_siphash_kat(cases: &[TestCase]) -> (String, EmitStats) {
    ...
    for case in cases {
        ...
        let (kind_str, fn_suffix, hash_call, assert_expr) = if alg.contains("SIPHASH128") {
            used_128 = true;
            (
                "SipHash-128 KAT",
                "siphash128",
                "SipHash128::hash(key, data).unwrap()",
                "mac.as_slice()",
            )
        } else if alg.contains("SIPHASH64") {
            (
                "SipHash-64 KAT",
                "siphash64",
                "SipHash::hash(key, data).unwrap()",
                "mac.to_le_bytes().as_slice()",
            )
        } else {
            stats.skipped_unsupported_alg += 1;
            continue;
        };
        write_doc(&mut body, case, kind_str);
        writeln!(body, "#[test]").unwrap();
        writeln!(body, "fn tc_line{}_{}() {{", case.line, fn_suffix).unwrap();
        writeln!(body, "    let key: &[u8] = {};", format_byte_slice(key)).unwrap();
        ...
        .unwrap();
        writeln!(body, "    let mac = {hash_call};").unwrap();
        writeln!(body, "    assert_eq!({assert_expr}, expected);").unwrap();
        writeln!(body, "}}\n").unwrap();
        stats.emitted += 1;
    }

    let mut out = String::new();
    let imports = if used_128 {
