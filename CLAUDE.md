...
- **Status**: Phase 92 complete + Phase T113 — TLS 1.3 key schedule & HKDF robustness tests (2739 tests)
...
│   ├── hitls-crypto/    # Cryptographic algorithms (feature-gated): AES, SM4, ChaCha20, SHA-2/3, SM3, HMAC, RSA, ECC, Ed25519/448, X25519/448, DH, DSA, SM2, SM9, PQC (ML-KEM/ML-DSA/SLH-DSA/XMSS/FrodoKEM/McEliece), DRBG, FIPS/CMVP, entropy health, hardware AES (709 tests + 15 Wycheproof, 31 ignored)
│   ├── hitls-tls/       # TLS 1.3/1.2 (91 cipher suites), DTLS 1.2, TLCP, DTLCP; 10 connection types (5 sync + 5 async via tokio); 15 TLS extensions; 10 callbacks; session cache, hostname verification, renegotiation, GREASE, custom extensions, NSS key logging (1229 tests)
...
# Run all tests (2739 tests, 40 ignored)
cargo test --workspace --all-features

# Run tests for a specific crate
cargo test -p hitls-crypto --all-features   # 709 tests (31 ignored) + 15 Wycheproof
cargo test -p hitls-tls --all-features      # 1229 tests

cargo test -p hitls-pki --all-features      # 349 tests (1 ignored)
cargo test -p hitls-bignum                  # 49 tests
cargo test -p hitls-utils                   # 61 tests
...
Phase 0–92 complete + Phase T73–T113 + Phase R102–R111 (2739 tests, 40 ignored). **100% C→Rust feature parity achieved. Architecture refactoring complete.**

### Completed Phases (Summary)

Implementation Phase 0–92 cover all crypto algorithms (48 modules), TLS 1.3/1.2 (91 cipher suites), DTLS 1.2, TLCP, DTLCP, PKI/X.509/CMS, FIPS/CMVP, entropy health testing, CLI (14 commands), and async I/O. Phase T73–T113 provide comprehensive unit test coverage across all modules.
...
- Phase T73–T113: CLI unit tests, async connection tests, cipher suite integration, codec/state machine edge cases, ECC point/AES soft/SM9 field arithmetic/McEliece vector, 0-RTT early data tests, async TLS 1.2 deep coverage, async TLCP + DTLCP connection types & tests, extension negotiation E2E tests, DTLS loss simulation & resilience tests, TLCP double certificate validation tests, SM9 tower field unit tests, SLH-DSA internal module unit tests, McEliece + FrodoKEM + XMSS internal module tests, proptest property-based tests + coverage CI, TLCP SM3 cryptographic path coverage, TLS 1.3 key schedule & HKDF robustness tests
