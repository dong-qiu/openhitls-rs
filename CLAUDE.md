...
- **Status**: Feature-complete C→Rust parity — Implementation / Testing / Refactoring / Performance phases all complete; 4300+ tests. **`DEV_LOG.md` is the single source of truth** for the exact phase index and test counts; this line stays deliberately approximate so it does not need a per-phase edit. (See `DEV_LOG.md` for the current phase head — the C→Rust test-migration tail, Phases J–M, is the most recent line of work.)
...
- **`strict` mode & scaling**: `strict` means a PR must be rebased onto the latest `main` before it can merge — so when one PR merges, the others queued behind it each need a rebase. This is fine at low PR volume (the manual rebase costs minutes). **Upgrade trigger**: if 3+ PRs are routinely queued and the rebase churn becomes a real time sink (e.g. several worktree slots producing PRs concurrently), adopt **GitHub Merge Queue** — add an `on: merge_group:` trigger to `ci.yml` and enable the queue in branch protection; it does the speculative rebase + CI automatically. Not worth the setup overhead below that volume.
...
- `CLAUDE.md` / `README.md` — **Do not** bump exact test counts or phase ranges here per phase. `DEV_LOG.md` is the single source of truth for those; CLAUDE.md's status line and README's counts are deliberately approximate ("4300+ tests"). Update them only when (a) a count drifts materially (roughly every few hundred tests), (b) a new phase *category* appears, or (c) workspace structure / protocol-algorithm tables genuinely change. Rationale: CLAUDE.md and README are the two most-edited shared files; keeping precise per-phase numbers out of them removes them from the cross-worktree merge-conflict path (every parallel PR otherwise collides on the same status line).
