...
| **SM2** | **Rust 2.7–6.9× faster** | Phase P10: specialized 4×u64 Montgomery field + precomputed comb table |
...
- **SM4-CBC**: Rust now **at parity** for encrypt (1.00×) and **1.22× faster** for decrypt. Phase P8 T-table optimization (fused S-box + L-transform into 4×u32 lookups + 3 XOR) yielded 2.4× block-level speedup. CBC decrypt benefits from parallelizable block processing.
...
| SM2 | Sign | 2,560 | 17,668 | **6.90** | **P10: 25.3× speedup, Rust now 6.9× faster than C!** |
| SM2 | Verify | 4,527 | 12,015 | **2.65** | **P10: 21.1× speedup, Rust now 2.65× faster than C!** |
| SM2 | Encrypt | 1,283 | 6,485 | **5.05** | **P10: 18.7× speedup, Rust now 5× faster than C!** |
| SM2 | Decrypt | 2,584 | 14,161 | **5.48** | **P10: 20.2× speedup, Rust now 5.5× faster than C!** |
...
- **SM2 (Rust 2.7–6.9× FASTER)**: Phase P10 applied the same specialized field arithmetic as P-256. SM2 is now dramatically faster in Rust — sign is 6.9× faster than C, verify is 2.65× faster. Previous gap of 3–6× has been completely reversed.
...
**Modular exponentiation** (Phase P7 CIOS Montgomery):
...
## 5. Performance Optimization Roadmap (Phase P1–P160)
...
|-------|-------------|-------------|--------|--------|--------|
| **P5** | P-256 深度优化 (预计算表 + 专用约简) | 16–32× → 1.5–2× | 2–3× | High | **Complete** |
| **P6** | ML-KEM SIMD NTT 向量化 | 6–18× | 2–3× | High | **Complete** |
| **P7** | BigNum CIOS 融合乘+约简 + 预分配缓冲 | 7–12× → 5.6–10× | 2–3× | High | **Complete** |
| **P8** | SM4 T-table 查表优化 | 2.2–2.4× → 1.0× | ~1× | Medium | **Complete** |
| **P9** | ML-DSA SIMD NTT 向量化 | 2–6× | NTT 2.3×; E2E ~1.02× | Medium | **Complete** |
| **P10** | SM2 专用字段算术 | 2.8–6.1× | 18–25× | Medium | **Complete** |
| **P11** | SHA-512 硬件加速 (ARMv8.2 SHA512) | 1.35× | ~1× | Low | Pending |
| **P12** | Ed25519 基点预计算表 | 2× | ~1.2× | Low | Pending |

---

### Phase P5 — P-256 深度优化 (预计算生成点表 + 专用约简) ✅ Complete
...
### Phase P6 — ML-KEM SIMD NTT 向量化 ✅ Complete
...
### Phase P7 — BigNum CIOS 融合乘+约简 + 预分配缓冲 ✅ Complete
...
### Phase P8 — SM4 T-table 查表优化 ✅ Complete
...
**Analysis**: SM4 goes from "C 2.2–2.4× faster" to "Rust at parity or 1.7× faster". The GCM improvement (3×) exceeds CBC (2.4×) because hardware-accelerated GHASH (ARMv8 PMULL, Phase P2) combines with optimized T-table SM4 to outperform C's software GHASH + hand-tuned SM4 assembly.

---

### Phase P9 — ML-DSA SIMD NTT 向量化 ✅ Complete
...
**Optimization plan** (mirrors Phase P5 approach for P-256):
...
| Handshake Type | Before P5 (Rust) | After Phase P5 | C Reference |
|---------------|---------------|----------------|-------------|
| **ECDHE-P256 + AES-128-GCM** | ~3.8 ms | **~0.23 ms** | 0.21 ms |
| **X25519 + AES-128-GCM** | ~0.018 ms | 0.018 ms (no change needed) | 0.020 ms |
| **ML-KEM-768 hybrid** | ~0.11 ms | ~0.025 ms (after P6) | 0.008 ms |
| **FFDHE-2048** | ~5.8 ms | **~4.4 ms** (P7 CIOS) | 0.82 ms |
...
**Phase P5** reduced ECDHE-P256 handshake from ~3.8 ms to ~0.23 ms, **within 1.1× of C** (0.21 ms).
