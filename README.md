...
> **100% C→Rust feature parity achieved** — 4154 tests, 68 fuzz targets, 5000+ Wycheproof vectors
...
| Test Infrastructure | ~20K | ~3.5K | **95%** | 4154 tests + Wycheproof + 68 fuzz targets (447 corpus) + OpenSSL TLS interop + security audit |
...
# Run all tests (4154 tests, 42 ignored)
cargo test --workspace --all-features

# Run tests for a specific crate
cargo test -p hitls-crypto --all-features   # 1487 tests (24 ignored)
cargo test -p hitls-tls --all-features      # 1507 tests
cargo test -p hitls-pki --all-features      # 444 tests
cargo test -p hitls-bignum                  # 95 tests (1 ignored)
cargo test -p hitls-utils                   # 78 tests
cargo test -p hitls-auth --all-features     # 49 tests
cargo test -p hitls-cli --all-features      # 165 tests (5 ignored)
cargo test -p hitls-integration-tests       # 261 tests (12 ignored)
