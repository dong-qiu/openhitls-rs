...
│   ├── hitls-tls/       # TLS 1.3/1.2/DTLS 1.2/TLCP/DTLCP, async I/O (tokio), custom extensions, NSS key logging, 53+ cipher suites (incl. TLS 1.2 AES-CCM RFC 6655/7251), hybrid KEM (X25519MLKEM768), TLS 1.3 SM4-GCM/CCM (RFC 8998), Record Size Limit, Fallback SCSV, OCSP stapling, SCT (620 tests)
...
| TLS 1.2 | `hitls-tls` | **Done** — 53 cipher suites (ECDHE/RSA/DHE_RSA/PSK/DHE_PSK/RSA_PSK/ECDHE_PSK key exchange, GCM/CBC/ChaCha20/CCM, Bleichenbacher protection), ALPN, SNI, session resumption, session ticket (RFC 5077), EMS (RFC 7627), ETM (RFC 7366), renegotiation indication (RFC 5746), mTLS, PSK (RFC 4279/5489), AES-CCM (RFC 6655/7251) |
...
| Feature | Standard | Status |
|---------|----------|--------|
| TLS 1.2 Handshake | RFC 5246 | **Done** (53 cipher suites: ECDHE/RSA/DHE_RSA/PSK/DHE_PSK/RSA_PSK/ECDHE_PSK, GCM/CBC/ChaCha20/CCM) |
| TLS 1.2 Cipher Suites (50+) | RFC 5246 | **Done** (53 suites: 14 ECDHE + 6 RSA + 7 DHE_RSA + 20 PSK + 6 CCM) |
...
| TLS Protocol | ~52K | ~14K | **100%** (TLS 1.3 + 1.2 + DTLS 1.2 + TLCP + DTLCP + X25519MLKEM768 + SM4-GCM/CCM + AES-CCM (RFC 6655/7251) + RSL/SCSV/OCSP/SCT + async I/O + key logging + custom extensions + all 5 FFDHE groups) | — |
...
| Test Infrastructure | ~20K | ~3.5K | **95%** (1790 tests + 5000+ Wycheproof vectors + 10 fuzz targets + security audit) | SDV compliance tests |
