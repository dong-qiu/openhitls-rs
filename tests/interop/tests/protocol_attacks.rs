...
fn test_cbc_mac_vs_padding_failure_indistinguishable() {
    ...
    let mut enc1 = RecordEncryptor12Cbc::new(&enc_key, &mac_key, 32).unwrap();
    let record1 = enc1
        .encrypt_record(ContentType::ApplicationData, b"payload")
        .unwrap();
    let mut dec_wrong_mac = RecordDecryptor12Cbc::new(&enc_key, &[0xCDu8; 32], 32).unwrap();
    ...
    let mut enc2 = RecordEncryptor12Cbc::new(&enc_key, &mac_key, 32).unwrap();
