...
mod tests {
    ...
    /// Build a valid TLCP ClientHello message for testing server error paths.
    fn build_valid_client_hello() -> Vec<u8> {
    ...
    #[test]
    fn test_tlcp_server_missing_enc_certificate() {
        let ch_msg = build_valid_client_hello();
    ...
    #[test]
    fn test_tlcp_server_missing_signing_key() {
        let ch_msg = build_valid_client_hello();
    ...
    #[test]
    fn test_tlcp_server_wrong_signing_key_type() {
        let ch_msg = build_valid_client_hello();
        let (_sign_privkey, sign_cert, _enc_privkey, enc_cert) = create_test_sm2_certs();

        // Server config with sign cert + enc cert but Ed25519 key (wrong type for TLCP)
        ...
        let msg = format!("{}", result.err().unwrap());
        assert!(
            msg.contains("TLCP signing key must be SM2"),
            "expected 'TLCP signing key must be SM2', got: {msg}"
        );
    }
}
