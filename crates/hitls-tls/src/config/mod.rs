...
/// Security operation types passed as the `op` parameter to [`SecurityCallback`].
pub mod security_op {
    /// Cipher suite check.
...
impl Default for TlsConfigBuilder {
    fn default() -> Self {
        Self {
            ...
            ticket_key_cb: None,
            security_cb: Some(Arc::new(default_security_callback)),
            security_level: 1,
...
mod tests {
    ...
    #[test]
    fn test_config_security_cb_default_present() {
        let config = TlsConfig::builder().build();
        assert!(config.security_cb.is_some());
    ...
    // -------------------------------------------------------
    // Default security callback enforcement tests
    // -------------------------------------------------------
    ...
    #[test]
    fn test_default_security_level1_rejects_anon() {
        use super::{default_security_callback, security_op};
        // DH_anon suite
        assert!(!default_security_callback(
            security_op::CIPHER_SUITE,
            1,
            CipherSuite::TLS_DH_ANON_WITH_AES_128_GCM_SHA256.0,
        ));
        // ECDH_anon suite
        assert!(!default_security_callback(
            security_op::CIPHER_SUITE,
    ...
    #[test]
    fn test_default_security_level1_rejects_old_versions() {
        use super::{default_security_callback, security_op};
        // SSL 3.0
        assert!(!default_security_callback(security_op::VERSION, 1, 0x0300));
        // TLS 1.0
        assert!(!default_security_callback(security_op::VERSION, 1, 0x0301));
        // TLS 1.1
        assert!(!default_security_callback(security_op::VERSION, 1, 0x0302));
        // TLS 1.2 allowed
        assert!(default_security_callback(security_op::VERSION, 1, 0x0303));
        // TLS 1.3 allowed
        assert!(default_security_callback(security_op::VERSION, 1, 0x0304));
        // DTLS 1.0 rejected
        assert!(!default_security_callback(security_op::VERSION, 1, 0xFEFF));
        // DTLS 1.2 allowed
        assert!(default_security_callback(security_op::VERSION, 1, 0xFEFD));
    }

    #[test]
    fn test_default_security_level1_allows_ecdhe_aes128() {
        use super::{default_security_callback, security_op};
        // ECDHE_RSA_AES_128_GCM = 128-bit, forward-secret → allowed at level 1
        assert!(default_security_callback(
            security_op::CIPHER_SUITE,
            1,
    ...
    #[test]
    fn test_default_security_level3_rejects_static_rsa() {
        use super::{default_security_callback, security_op};
        // Static RSA = not forward-secret → rejected at level ≥ 3
        assert!(!default_security_callback(
            security_op::CIPHER_SUITE,
            3,
            CipherSuite::TLS_RSA_WITH_AES_128_GCM_SHA256.0,
        ));
        // ECDHE = forward-secret → still allowed
        assert!(default_security_callback(
            security_op::CIPHER_SUITE,
            3,
            CipherSuite::TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256.0,
    ...
    #[test]
    fn test_default_security_level4_rejects_sha1_mac() {
        use super::{default_security_callback, security_op};
        // ECDHE_RSA_AES_128_CBC_SHA uses SHA-1 MAC → rejected at level ≥ 4
        assert!(!default_security_callback(
            security_op::CIPHER_SUITE,
            4,
            CipherSuite::TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA.0,
        ));
        // ECDHE_RSA_AES_256_GCM_SHA384 uses AEAD → allowed
        assert!(default_security_callback(
            security_op::CIPHER_SUITE,
            4,
    ...
    #[test]
    fn test_default_security_level5_rejects_128bit() {
        use super::{default_security_callback, security_op};
        // AES-128 = 128-bit strength → rejected at level 5 (needs 256)
        assert!(!default_security_callback(
            security_op::CIPHER_SUITE,
            5,
            CipherSuite::TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256.0,
        ));
        // AES-256 = 256-bit strength → allowed
        assert!(default_security_callback(
            security_op::CIPHER_SUITE,
            5,
            CipherSuite::TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384.0,
    ...
    #[test]
    fn test_default_security_tls13_forward_secret() {
        use super::{default_security_callback, security_op};
        // All TLS 1.3 suites are forward-secret → allowed even at level 3
        assert!(default_security_callback(
            security_op::CIPHER_SUITE,
            3,
    ...
    #[test]
    fn test_default_security_level4_tlcp_version() {
        use super::{default_security_callback, security_op};
        // TLCP 1.1 (0x0101): allowed at level < 4, rejected at level ≥ 4
        assert!(default_security_callback(security_op::VERSION, 3, 0x0101));
        assert!(!default_security_callback(security_op::VERSION, 4, 0x0101));
    }

    #[test]
    fn test_security_level_bits() {
        use super::security_level_bits;
        assert_eq!(security_level_bits(0), 0);
        assert_eq!(security_level_bits(1), 80);
        assert_eq!(security_level_bits(2), 112);
        assert_eq!(security_level_bits(3), 128);
        assert_eq!(security_level_bits(4), 192);
        assert_eq!(security_level_bits(5), 256);
        // Level > 5 clamps to level 5
        assert_eq!(security_level_bits(99), 256);
    }
}
