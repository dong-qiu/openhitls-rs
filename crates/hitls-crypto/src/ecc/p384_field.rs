...
impl P384FieldElement {
    ...
    pub fn add(&self, other: &Self) -> Self {
        ...
        if carry != 0 || cmp_u384(&r, &P) != Ordering::Less {
            sub_borrow_u384(&mut r, &P);
        }
        Self(r)
    }
    ...
    pub fn sub(&self, other: &Self) -> Self {
        let (mut r, borrow) = sub_u384(&self.0, &other.0);
        if borrow != 0 {
            add_carry_u384(&mut r, &P);
        }
        Self(r)
    }
...
fn p384_mont_reduce(mut t: [u64; 12]) -> P384FieldElement {
    ...
    for i in 0..6 {
        let m = (t[i] as u128).wrapping_mul(N0 as u128) as u64;

        // j=0: P[0] = 0x00000000FFFFFFFF
        let p = (m as u128) * (P[0] as u128) + (t[i] as u128);
        ...
        let p = (m as u128) * (P[1] as u128) + (t[i + 1] as u128) + (carry as u128);
        t[i + 1] = p as u64;
        carry = (p >> 64) as u64;

        // j=2: P[2] = 0xFFFFFFFFFFFFFFFE
        let p = (m as u128) * (P[2] as u128) + (t[i + 2] as u128) + (carry as u128);
        t[i + 2] = p as u64;
        carry = (p >> 64) as u64;

        // j=3: P[3] = 0xFFFFFFFFFFFFFFFF
        let p = (m as u128) * (P[3] as u128) + (t[i + 3] as u128) + (carry as u128);
        t[i + 3] = p as u64;
        carry = (p >> 64) as u64;

        // j=4: P[4] = 0xFFFFFFFFFFFFFFFF
        let p = (m as u128) * (P[4] as u128) + (t[i + 4] as u128) + (carry as u128);
        t[i + 4] = p as u64;
        carry = (p >> 64) as u64;

        // j=5: P[5] = 0xFFFFFFFFFFFFFFFF
        let p = (m as u128) * (P[5] as u128) + (t[i + 5] as u128) + (carry as u128);
        t[i + 5] = p as u64;
        carry = (p >> 64) as u64;
    ...
    if overflow != 0 || cmp_u384(&r, &P) != Ordering::Less {
        sub_borrow_u384(&mut r, &P);
    }
...
// ========================================================================
// 384-bit unsigned arithmetic helpers
// ========================================================================

/// 384-bit addition: returns (result, carry).
...
/// 384-bit subtraction: returns (result, borrow). Borrow is 1 if a < b.
fn sub_u384(a: &[u64; 6], b: &[u64; 6]) -> ([u64; 6], u64) {
    let mut r = [0u64; 6];
...
/// Compare two 384-bit numbers (little-endian limb order).
...
/// In-place 384-bit addition: a += b, ignoring overflow.
fn add_carry_u384(a: &mut [u64; 6], b: &[u64; 6]) {
    let mut carry = 0u64;
    for i in 0..6 {
        let sum = (a[i] as u128) + (b[i] as u128) + (carry as u128);
        a[i] = sum as u64;
...
/// In-place 384-bit subtraction: a -= b, ignoring underflow.
...
mod tests {
    ...
    #[test]
    fn test_cmp_u384_helper() {
        let a = [1u64, 0, 0, 0, 0, 0];
        let b = [2u64, 0, 0, 0, 0, 0];
        assert_eq!(cmp_u384(&a, &b), Ordering::Less);
        assert_eq!(cmp_u384(&b, &a), Ordering::Greater);
        assert_eq!(cmp_u384(&a, &a), Ordering::Equal);

        let c = [0u64, 0, 0, 0, 0, 1];
        let d = [u64::MAX, u64::MAX, u64::MAX, u64::MAX, u64::MAX, 0];
        assert_eq!(cmp_u384(&c, &d), Ordering::Greater);
    }
