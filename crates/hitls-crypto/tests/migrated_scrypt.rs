...
/// Test vectors for rfc7914 scrypt #1
/// C source: SDV_CRYPT_EAL_KDF_SCRYPT_FUN_TC001 (line 8, scrypt KAT)
#[test]
fn tc_line8_scrypt() {
    let password: &[u8] = &[];
    let salt: &[u8] = &[];
    ...
    let actual = scrypt(password, salt, 16, 1, 1, expected.len()).unwrap();
    assert_eq!(actual.as_slice(), expected);
}

/// Test vectors for rfc7914 scrypt #2
/// C source: SDV_CRYPT_EAL_KDF_SCRYPT_FUN_TC001 (line 11, scrypt KAT)
#[test]
fn tc_line11_scrypt() {
    ...
    let actual = scrypt(password, salt, 1024, 8, 16, expected.len()).unwrap();
    assert_eq!(actual.as_slice(), expected);
}

/// Test vectors for rfc7914 scrypt #3
/// C source: SDV_CRYPT_EAL_KDF_SCRYPT_FUN_TC001 (line 14, scrypt KAT)
#[test]
fn tc_line14_scrypt() {
    let password: &[u8] = &[
        0x70, 0x6c, 0x65, 0x61, 0x73, 0x65, 0x6c, 0x65, 0x74, 0x6d, 0x65, 0x69, 0x6e,
    ];
    let salt: &[u8] = &[
        0x53, 0x6f, 0x64, 0x69, 0x75, 0x6d, 0x43, 0x68, 0x6c, 0x6f, 0x72, 0x69, 0x64, 0x65,
    ];
    let expected: &[u8] = &[
    ...
    let actual = scrypt(password, salt, 16384, 8, 1, expected.len()).unwrap();
    assert_eq!(actual.as_slice(), expected);
}

// Generation summary: 3 emitted / 17 API-surface skipped (N/A in Rust) / 0 unknown / 0 unsupported alg / 20 total C cases.
